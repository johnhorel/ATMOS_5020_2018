...
In the file explorer, double click the file or right-click and select `open in [browser]`. The file when opened in will show only a blank screen. This is expected because the file doesn't have anything in it.
...
|`<hr>`| Inserts a horizontal line across the page to separate content. **No closing tag**. <hr>|
