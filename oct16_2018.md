...
The “Shell” is simply the layer on top of the operating system that provides a basic human-OS interface.
  ...
  - Science and engineering
  # Linux on Mac
  	Step 1: Open x-term window using terminal or XQuartz
  	Step 2: echo $SHELL
  	Step 3: if not tcsh, then type exec tcsh or just tcsh
  	Step 4: do stuff …
  	Step X: When done, type exit
  ...
  % ls (don’t type the %, I’m showing it is a command)
  ...
  % pwd
   where you are on the file system
  % cd
  Return to your home directory
  % df -m
  what disks are on the system and the amount of space available in kilobytes
  ...
  % man
  formats and displays on-line manual pages
  % history
  List prior commands
  % !!
  Previous command
  ...
  % cd dirname/
  Change to dirname directory relative to the location you are currently in
