...
> If you need help, type `help()`. You can also get help documents for anything else when you include the function inside the parentheses, e.g. `help(max)`. 
To exit the `help()`, type `q` or `ctrl-c`. Alternatively, search online for how to do things.

## How to exiting the IDLE
...
1. This is known as a **for loop**. The following code block executes for changing values of `i`. Each value of `i` is iterated by the sequence `range(0,3)`. Type `help(range)` to read about what range does. During each loop, the value of `i` is reassigned 0, 1, 2, 3, and 4. The last number, 5, is excluded, as described in the help documents. The colon `:` indicates that a code block follows. Note that each line in the code block is indented exactly 4 spaces. This characteristic of Python helps make code readable.

2. For each value of `i`, assign the variable `x` to double the value of `i`.

4. `print()` is a function that takes input (x in this case) and writes to the screen.
