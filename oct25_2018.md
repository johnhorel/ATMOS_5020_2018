...
|--|--|--|
|Anaconda|https://www.anaconda.com/download/| **RECOMMENDED**. Comes with a lot of extra packages for standard data analysis. Some packages you will use and others you probably won't. |
|Miniconda|https://conda.io/miniconda.html| Same as Anaconda, but without the fluff packages you won't use. Instead, you need to install the packages you will need individually, e.g. `conda install numpy`. If you want to save space on your computer, try this. _Note: I haven't been able to get Python Notebooks to work with this installation._|
...
The following are the library packages we will use in class: `numpy`, `scipy`, `matplotlib`, `pandas`, and `Basemap`. If you installed Python with **Anaconda** or **Miniconda**, you can install or update these packages with the `conda` command in the terminal or Anaconda Command Prompt:
